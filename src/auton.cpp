...
void redLong(){
  ...
  //Align with wall
  backwardPID(6800, 500, 50, 3, 60);
  turnRight(90);
  driveIntoWall(300, 50);
  intake.stop(hold);

  //Go for middle home row
  forwardPID(10600, 450, 50, 4.25, 80);
  turnRight(179);
  pneum.close();
  ...
  rollerSystem.stop(coast);
  stage3.rotateFor(fwd, 2, rev, 80, percent, false);
  stage1.stop(hold);
  intake.rotateFor(reverse, 2, rev, 100, percent, false);
  wait(250, msec);  
  
  intake.stop(coast);
  //stage3.rotateFor(fwd, 3, rev, 100, percent, false);
...
void redLongMiddle(){ 
  ...
  while(blue == true){
    ...
    intake.spin(fwd, 100,pct);
    if(beamBot.value() == 1 || Optical.hue() > 120 || t > 3000){
      blue = false;
    }
    t+=15;
    wait(15,msec);
  ...
  rollerSystem.stop(coast);
  intake.rotateFor(reverse, 2, rev, 100, percent);
  intake.stop(coast);
  pneum.close();
