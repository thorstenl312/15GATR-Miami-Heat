...
void usercontrol(void) {
  ...
  task bB(badBallControl); //COMMENT OUT IF SKILLS
  //task release(skillsRelease);
...
int badBallControl(){
  while(true){
    ...
    else if(((Optical.hue() > 110 && badBall == false) && !side) || ((Optical.hue() < 30 && badBall == false) && side)){
      badBall = true;
    }
    wait(20,msec);
  }
